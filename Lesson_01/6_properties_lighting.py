...
# Function to add light and its corresponding sphere
def add_light(renderer: vtkRenderer, color, position):
    # Create a light
    light = vtkLight()
    light.SetColor(color)
    light.SetPosition(position)
    light.SetFocalPoint(0.0, 0.0, 0.0)  # Pointing towards the origin
    renderer.AddLight(light)

    # Create a sphere to represent the light
    light_sphere = vtkSphereSource()
    light_sphere.SetRadius(0.5)
    light_sphere.SetThetaResolution(20)
    light_sphere.SetPhiResolution(20)

    sphere_mapper = vtkPolyDataMapper()
    sphere_mapper.SetInputConnection(light_sphere.GetOutputPort())

    sphere_actor = vtkActor()
    sphere_actor.SetMapper(sphere_mapper)
    sphere_actor.SetPosition(position)
    sphere_actor.GetProperty().SetColor(color)  # Set the sphere's color
    sphere_actor.GetProperty().LightingOff()  # Disable lighting effects on the sphere
...
def main(shape_type):
    ...
    # Add lights and their spheres
    add_light(ren, (1, 0, 0), (-5, 0, 0))  # Red light
    add_light(ren, (0, 1, 0), (0, 0, -5))  # Green light
    add_light(ren, (0, 0, 1), (5, 0, 0))   # Blue light
    add_light(ren, (1, 1, 0), (0, 0, 5))   # Yellow light
