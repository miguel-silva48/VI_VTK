...
def main(shape_type):
    ...
    # cam1 = vtkCamera()
    # cam1.SetPosition(10,0,0) # Changes the initial camera position
    # cam1.SetViewUp(0,1,0) # Changes the initial camera orientation
    # ren.SetActiveCamera(cam1)

    # Use the existing camera for the same effect
    camera = ren.GetActiveCamera()
    camera.SetPosition(10,0,0)
    camera.SetViewUp(0,1,0)
    camera.SetParallelProjection(True)
    # Parallel projection maintains the size of objects, regardless of distance
    # Unlike perspective projection, where objects appear smaller as they move away
    #* Note: better seen with the cube and with Wireframe on
